    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const regOnly = response.data.filter(clientData => clientData.clientType === "LGU");
                setclient(regOnly);
            ...
            cell: (row) => (
                <div className="d-flex align-items-center gap-2">
                  <button type="button" className="btn p-0 border-0 " onClick={() => deletRequest(row._id)}><i className="bi bi-trash text-danger "></i></button>
                    ...
                    />
                    <Link to={`/Dco/requestDetails/${row._id}`} state={{from: '/Dco/LGU/'}} type="button" className="btn p-0 border-0"><i class="bi bi-eye"></i></Link>
                </div>
              ),
        },
    ];
