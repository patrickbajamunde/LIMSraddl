...
export default function Reports() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://localhost:8003/api/report/userReports", {
    ...
    const updateStatus = async (roaId, reportId) => {
        try {
            ...
            const response = await axios.put(
                `http://localhost:8003/api/report/update/report/${roaId}`,
                { status: "Released" },
                { withCredentials: true }
            );
    ...
    const deleteReport = async (roaDataId) => {
        try {
            const confirmDelete = window.confirm("Are you want to delete this report?")
            if (!confirmDelete) return;

            await axios.delete(`http://localhost:8003/api/report/delete/report/${roaDataId}`, {
                withCredentials: true,
            })

            setclient(prev => prev.filter(item => item._id !== roaDataId));
            setFilteredData(prev => prev.filter(item => item._id !== roaDataId));
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="d-flex justify-content-center">
                    <button className={`btn btn-sm ${row.status === "Released" ? "btn-success" : row.status === "For release" ? "btn-primary" : "btn-secondary"} text-white`} onClick={() => updateStatus(row._id, row.reportId)}>
                        {row.status}
                    </button>
                </div>
            )
        },
        {
            ...
            cell: (row) => (
                <div className="d-flex align-items-center gap-2">
                    <button type="button" className="btn p-0 border-0 " onClick={() => deleteReport(row._id)}><i className="bi bi-trash text-danger "></i></button>
                    <Link
                    ...
                    />
                    <Link to={`/Dco/reportDetails/${row._id}`} type="button" className="btn p-0 border-0"><i class="bi bi-eye"></i></Link>
                </div>
            ),
        },
    ];
